...
root.geometry('307x192')
fr1 = tk.Frame(root,bg= "#333333",width=350,height=30,)
...
fr3=tk.Frame(root,bg='lightgray',width=130,height=100)
fr3.propagate(False)
...
#----------------traffic------------------
def traffic():
    traffic_lb1 = tk.Label(fr1, text='運行状況:',bg='gainsboro',font=("", 11))
    ...
    print(odakyu)
    if "平常"in odakyu:
        traffic_lb2 = tk.Label(fr1, text="小田急線：平常運転",bg='skyblue',font=("", 11))
        traffic_lb2.grid(column=1,row=0,sticky=tk.W+tk.E)
    else:
        traffic_lb2 = tk.Label(fr1, text=odakyu, foreground='white',bg='red',font=("", 11))
        traffic_lb2.grid(column=1, row=0,sticky=tk.W+tk.E)
    # ----------------Keio------------------
    ...
    print(str(keio)+"hi")
    if "平常"in keio:
        traffic_lb3 = tk.Label(fr1, text="京王線：平常運転", bg='plum',font=("", 11))
        traffic_lb3.grid(column=2, row=0,sticky=tk.W+tk.E)
    else:
        traffic_lb3 = tk.Label(fr1, text=keio, foreground='white', bg='red',font=("", 11))
        traffic_lb3.grid(column=1, row=1, sticky=tk.W+tk.E)

    up_date= strftime('%H:%M:%S')
    time_label=tk.Label(fr1,text=up_date,bg='gainsboro',font=("", 11))
...
def weather():
    ...
    if weather =="晴" or weather=="晴時々曇":
        ...
        txt='⛅️'
        bg="gray"
    elif weather =="曇のち雨"or weather=="雨のち曇":
        txt="☔️"
        bg="royalblue"
    elif weather =="雨":
        txt="☔️"
    ...
    rain_prob= tk.Label(fr3,text="|"+str(prb1)+"|"+str(prb2)+"|"+str(prb3)+"|"+str(prb4),
                        font=("",13),bg='lightgray')
    rain_prob.pack(anchor=tk.W)
weather()
...
def news():
    ...
    #latest_news = soup.find_all('em',class_='title')
        print(content.text)
        main_news=tk.Label(fr4,text=":"+str(content.text),font=("",15,"bold"),bg="skyblue4",foreground='white')
